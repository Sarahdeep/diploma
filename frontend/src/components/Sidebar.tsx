...
import { ListTree, UploadCloud, Settings, Map } from 'lucide-react';

interface SidebarProps {
  active: 'upload' | 'species' | 'analysis' | 'map';
  onSelect: (section: SidebarProps['active']) => void;
}

export function Sidebar({ active, onSelect }: SidebarProps) {
  const items: { key: SidebarProps['active']; icon: React.ReactNode; label: string }[] = [
    { key: 'upload', icon: <UploadCloud className="mr-2 h-4 w-4" />, label: 'Администрирование' },
    { key: 'species', icon: <ListTree className="mr-2 h-4 w-4" />, label: 'Виды' },
